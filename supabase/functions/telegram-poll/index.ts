// Rizztalk Telegram Bot — main polling + command handlers
// Triggered every minute by pg_cron. Loops getUpdates with long-poll up to 50s
// for ~55s total runtime, fitting within the 60s cron interval.

import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.0";
import { sendMessage, sendKeyboard, removeKeyboard, getUpdates } from "../_shared/telegram.ts";
import { PROVINCES_ID, PRESET_INTERESTS, findProvinceByText } from "../_shared/provinces-id.ts";
...
// In-memory conversational state per chat (resets on cold start, OK for now)
type Step =
  | { name: "idle" }
  | { name: "set_alias" }
  | { name: "set_gender" }
...
async function tryMatch(supabase: ReturnType<typeof getSupabase>, profile: Profile) {
  ...
  for (const c of candidates) {
    // Gender preference filtering (mutual respect when premium)
  ...
  const { data: conv, error: convErr } = await supabase
    ...
    .single();
  if (convErr) {
    // Likely lost race (partner already matched). Return null so we keep waiting.
    return null;
  }

  // Mark both as matched & remove from queue
...
export async function processUpdate(supabase: ReturnType<typeof getSupabase>, update: TgUpdate) {
  ...
  // Free text — onboarding step OR relay to active chat
